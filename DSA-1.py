...
def linear_probing_insert(H, value, n):       # H is hash-table(list)
    index = Hash(value, n)
    i = 0
    ...
    if i == n:
        ...
        H[(index + i) % n] = value
        print(f"Inserted {value} at index {(index + i) % n}")
...
def search_linear_probing(H, value, n):
    ...
    if H[(index + i) % n] == value:
        print(f"Found {value} at index {(index + i) % n} with {i + 1} comparisons.")
    else:
...
def delete_linear_probing(H, value, n):
    ...
    if H[(index + i) % n] == value:
        H[(index + i) % n] = -1
        print(f"Deleted {value} from index {(index + i) % n}")
    else:
        print("Number not found!")
...
def quadratic_probing_insert(H, value, n):
    ...
    if i == n:
        ...
        H[(index + i * i) % n] = value
        print(f"Inserted {value} at index {(index + i * i) % n}")
...
def search_quadratic_probing(H, value, n):
    ...
    if H[(index + i * i) % n] == value:
        print(f"Found {value} at index {(index + i * i) % n} with {i + 1} comparisons.")
    else:
        print("Number not found!")
...
def delete_quadratic_probing(H, value, n):
    ...
    if H[(index + i * i) % n] == value:
        H[(index + i * i) % n] = -1
        print(f"Deleted {value} from index {(index + i * i) % n}")
    else:
        print("Number not found!")
...
H1 = [-1] * n  # Linear Probing                    # In Python, multiplying a list by a number replicates the list that many times. so (*n)
H2 = [-1] * n  # Quadratic Probing

while True:
    print("\n___MENU___")
    print("1. Insert using Linear Probing")
    print("2. Search using Linear Probing")
    print("3. Delete using Linear Probing")
    print("4. Insert using Quadratic Probing")
    print("5. Search using Quadratic Probing")
    print("6. Delete using Quadratic Probing")
    print("7. Display hash tables")
    print("8. Exit")
